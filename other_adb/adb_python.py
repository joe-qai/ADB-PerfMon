...
# excute cmd
# python adb_python.py 2 -r 2 -p D:\\adt-bundle-windows\\sdk\\platform-tools\\adb.exe -o D:\\out.json
...
def main(argv):
    ...
    cmd_packages = shlex.split(path + ' shell dumpsys package -f p |findstr "com.hcp.flaget"')
    ...
    while (not interrupt and (keep_running or elapsed_time <= running)):
        try:
            ...
            # Memory Info
            memInfo = getMemInfo()
            print("总内存信息：".format(memInfo))
            mem_tot = float(memInfo[1])
            mem_free = float(memInfo[4])
            print("剩余内存信息：".format(memInfo))
