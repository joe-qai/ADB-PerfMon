...
# excute cmd
# python adb_python.py 2 -r 2 -o D:\\out.json
...
def getCmdline(pid):
    cmd = 'adb shell cat proc/' + pid + '/cmdline'
    out = getCmdOutput(cmd)
    return out.strip('\0')
...
def programExists(prog_name):
    cmd = 'adb shell which ' + prog_name
    out = getCmdOutput(cmd)
    return out


def getStatFromTop(args):
    cmd = 'adb shell top ' + args
    out = getCmdOutput(cmd)
    if (len(out) > 4):
...
def getDiskStats():
    cmd = 'adb shell dumpsys diskstats'
    out = getCmdOutput(cmd)
    line = ''
    if out: line = out.splitlines()[2].split()
    return line
...
def main(argv):
    ...
    #################### Shell Commands ###################
    cmd_top = shlex.split('adb shell top -n 2 -d 1')
    cmd_packages = shlex.split('adb shell dumpsys package -f p |findstr "com.hcp.flaget"')
    ...
    while (not interrupt and (keep_running or elapsed_time <= running)):
        try:
            ...
            print("总内存大小：{}".format(mem_tot))
            mem_free = float(memInfo[4])
            print("剩余内存大小：{}".format(mem_free))
            mem_used = mem_tot - mem_free
            print("使用内存大小：{}".format(mem_free))

            # Storage Info
            diskinfo = getDiskStats()
            print("磁盘信息：{}".format(diskinfo))
