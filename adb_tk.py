...
def monkey_app():
	...
	if device_status =='device':
		try:
			...
			adb_monkey(pkname=packname,s_num=s,throttle=interval_times,pct_touch=click,pct_motion=sliding,pct_trackball=track,pct_nav=base_nav,pct_majornav=main_nav,pct_syskeys=syso,pct_appswitch=acti,pct_flip=kboard,pct_anyevent=others,times=event_times,logfilepath=log)
		except :
...
def cpu_app():
	...
	if device_status =='device':
		...
		for i in range(int(xing)):
			...
			netflow_t.insert(tkinter.END,('总流量：%sk,上传流量:%sk,下载流量：%sk'%(netflow_sum,rescv,send)))
			LOG.info('第%s次：总流量：%sk,上传流量:%sk,下载流量：%sk' % (i, netflow_sum,rescv,send))
		...
		get_cpu(times=times_list,start_cpu=cpu_list,recv_list=rescv_list,send_list=send_list,total_list=total_list,Pass_list=pass_list)
...
if __name__ == '__main__':
	...
	try:
		...
		if status_shebei =='device':
			...
			tkinter.Label(root,text='性能参数展示',fg='red',font=("黑体", 15, "bold"),).grid(row=1,column=3)
			...
			tkinter.Label(root,text='参数次数:',justify=LEFT).grid(row=1,column=5)
...
# 			tkinter.Label(root,text='启动时间测试',fg='red',height=2,font=("黑体", 15, "bold")).grid(row=8,column=3)
			tkinter.Label(root,text='启动测试包名:',justify=LEFT).grid(row=9,column=1)
			baoming_t=tkinter.Text(root,height=1,width=30)
			baoming_t.grid(row=9,column=2)
			baoming_t.insert('0.0', "com.chutzpah.yasibro.test")
			tkinter.Label(root,text='测试包Activity:',justify=LEFT).grid(row=9,column=3)
			activ_t=tkinter.Text(root,height=1,width=30)
			activ_t.grid(row=9,column=4)
			activ_t.insert('0.0', "com.chutzpah.yasibro.main.view.MainActivity")
			tkinter.Label(root,text='测试次数:').grid(row=9,column=5)
			num=[10,20,30,50,100]
			times_act=ttk.Combobox(root,values=num,width=5) #state='readonly',只读不可手输
			times_act.current(0)
			times_act.grid(row=9,column=6)
			
			tkinter.Label(root,text='启动时间展示:',justify=LEFT).grid(row=10,column=1)
			e1 = tkinter.Text(root,width=30,height=5, state="disabled")
			e1.grid(row=10,column=2,padx=20,pady=30)
			
			screenshot_btn = tkinter.Button(root, text='截图', width=8, height=3, font=("黑体", 16, "bold"), fg='red', command=ScreenShot)
...
# 			tkinter.Label(root,text='Monkey测试',fg='red',font=("黑体", 15, "bold")).grid(row=11,column=4)
			...
			tkinter.Label(root,text='主要导航事件百分比:',justify=LEFT).grid(row=13,column=1)
			...
			tkinter.Label(root,text='系统按键百分比:',justify=LEFT).grid(row=16,column=1)
			system_event=tkinter.Text(root,height=1,width=30)
			system_event.grid(row=16,column=2)
			system_event.insert('0.0',2)
			
			tkinter.Label(root,text='Activity启动事件百分比:',justify=LEFT).grid(row=15,column=3)
			...
			tkinter.Label(root,text='键盘唤出隐藏事件百分比:',justify=LEFT).grid(row=13,column=3)
			keyboard_event=tkinter.Text(root,height=1,width=30)
			keyboard_event.grid(row=13,column=4)
			keyboard_event.insert('0.0',1)
			
			tkinter.Label(root,text='其他事件百分比:',justify=LEFT).grid(row=17,column=3)
			...
			tkinter.Label(root,text='日志存放路径:',justify=LEFT).grid(row=18,column=3)
